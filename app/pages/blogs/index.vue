<template>
  <div>
    <PageHero
      title="News & Insights"
      subtitle="Stay updated with our latest industry insights, company news, and technological advancements in the ITS sector."
      label="Our Blog"
    />
    ...
    <section class="py-20 bg-slate-50 min-h-screen">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          <article v-for="blog in blogs" :key="blog.id" class="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-blue-900/5 transition-all duration-300 group flex flex-col">
            <NuxtLink :to="`/blogs/${blog.slug}`" class="block h-56 bg-slate-100 relative overflow-hidden">
            ...
            <div class="p-6 flex-1 flex flex-col">
              ...
              <NuxtLink :to="`/blogs/${blog.slug}`" class="block group-hover:text-blue-600 transition-colors">
                <h3 class="text-xl font-bold text-slate-900 mb-3 leading-tight">{{ blog.title }}</h3>
              </NuxtLink>
...
useSeoMeta({
  title: 'Blog & Insights — Asian Traffic Technologies Ltd',
  description: 'Read the latest news, technological insights, and company updates from Asian Traffic Technologies Ltd.',
})
</script>
